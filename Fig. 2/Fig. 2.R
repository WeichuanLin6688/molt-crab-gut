...
ann <- read.delim('~/Desktop/categorize/KO1-5.txt',header = T)#Read the KEGG classification database
...
otu <- read.delim('~/Desktop/SCI projects/S6_iMeta Omics/molt meta stats/enzyme.txt',row.names = 1,check.names = F)
group <- read.csv('~/Desktop/SCI projects/S6_iMeta Omics/molt meta stats/group meta.csv')#[-12,]
...
group <- read.csv('~/Desktop/molt github/Fig. 2/group meta.csv',row.names = 1)
a <- subset(read.csv('~/Desktop/molt github/Fig. S4/alpha stability.csv',row.names = 1),tissue=='Midgut')
...
library(FEAST)
comm <- read.delim('~/Desktop/molt github/Fig. S10/ASV_table_norm feast.txt',row.names = 1,check.names = F)
