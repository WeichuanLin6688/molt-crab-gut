...
source('~/Desktop/molt github/molt plot/alpha/alpha.R')
...
for (i in site) {
	...
	if (i== 'Mixed gut') {
		...
		otu <- otu[,colnames(otu) %in% group1$SampleID]
		ann <- read.delim('~/Desktop/molt github/molt plot/taxonomy_mix gut.txt',header = T,check.names = F)
	}	
	if (i=='Whole gut') {
		otu<- read.delim('~/Desktop/molt github/molt plot/ASV_whole gut.txt',row.names = 1,header = T,check.names = F)
		otu <- otu[,colnames(otu) %in% group1$SampleID]
		ann <- read.delim('~/Desktop/molt github/molt plot/taxonomy_whole gut.txt',header = T,check.names = F)
	}
...
#Finally, these plots are arranged into fig.S2 in AI software and further modify legends.