...
##fig. 1A----
library(splines)  
train.cv.mean <- read.csv('~/Desktop/molt github/Fig. 1 and Fig. S11-S12/mc_train.cv.mean.csv')
...
imp <- read.delim('~/Desktop/molt github/Fig. 1 and Fig. S11-S12/Mc importance_otu.txt',row.names = 1)
...
n1 <- read.csv('~/Desktop/molt github/Fig. 1 and Fig. S11-S12/mc rf NMDS.csv',row.names = 1)
...
#ggvenn
library(ggvenn)
df <- read.delim('~/Desktop/molt github/Fig. 1 and Fig. S11-S12/mc importance_otu.txt',check.names = F,row.names = 1)
df1 <- read.csv('~/Desktop/molt github/Fig. 1 and Fig. S11-S12/mc imp weight.csv',check.names = F,row.names = 1)
...
group <- read.csv("~/Desktop/molt github/molt plot/groupMOLT mc.csv",row.names = 1)
group <- subset(group,!time=='CK')
ann <- read.csv('~/Desktop/molt github/Fig. 1 and Fig. S11-S12/last classify.csv')
...
library(splines) 
source('~/Desktop/molt github/molt plot/theme_linweichuan.R')
train.cv.mean <- read.csv('~/Desktop/molt github/Fig. 1 and Fig. S11-S12/hc_train.cv.mean.csv')
p1 <- ggplot(train.cv.mean, aes(Group.1, x)) +
	geom_smooth(se = F,	method = 'lm', formula = y~ns(x,7),colour = "black",size=0.3) +##1F78B4
	geom_point(size=1.5,shape=19,stroke=0)+
...
##fig. S11B----
source('~/Desktop/molt github/molt plot/theme_linweichuan.R')
n1 <- read.csv('~/Desktop/molt github/Fig. 1 and Fig. S11-S12/hc rf NMDS.csv')
n1$time1 <- factor(n1$time1)
p4 <- ggplot(n1,aes(Dim.1,Dim.2))+
	geom_point(aes(color=time1),shape=19,size=2,stroke=0)+
...
##fig. S11C----
df <- read.delim('~/Desktop/molt github/Fig. 1 and Fig. S11-S12/hc importance_otu.txt')
df1 <- read.csv('~/Desktop/molt github/Fig. 1 and Fig. S11-S12/hc imp weight.csv',check.names = F,row.names = 1)
a1 <- list("Stage"=rownames(df),
					 "Crab weight"=df1$ASV_ID)
s1 <- ggvenn(a1,c("Stage","Crab weight"),show_percentage = F,
						 stroke_color = 'white',stroke_alpha = 0.5,stroke_size = 0.3,digits=1,
...
##fig. S11D----
library(reshape2)
df <- read.delim('~/Desktop/molt github/Fig. 1 and Fig. S11-S12/hc importance_otu.txt',check.names = F,row.names = 1)
df1 <- read.csv('~/Desktop/molt github/Fig. 1 and Fig. S11-S12/hc imp weight.csv',check.names = F,row.names = 1)
diff <- setdiff(rownames(df),rownames(df1))
ann <- read.delim('~/Desktop/molt github/molt plot/taxonomy_qc mc hc.txt')
otu <- read.delim("~/Desktop/molt github/molt plot/ASV_34848.txt",check.names = F,row.names = 1,header = T)
