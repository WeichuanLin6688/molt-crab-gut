#fig. S1A gut part ggtree------------------------------
library(vegan);library(ggtree);library(ggplot2)
otu <- read.delim("~/Desktop/molt github/molt plot/ASV_table all.txt",row.names = 1,header = T,check.names = F)
group<-read.delim("~/Desktop/molt github/molt plot/groupALL.txt",header = T)
...
groups1 <- subset(group,tissue=='Foregut')
set.seed(123)
tsne_out <- Rtsne(otu3,
									pca=F,perplexity=(nrow(otu3)-1)/3,theta=0.45)#you can adjust perplexity and theta value
tsne_res <- data.frame(tsne_out$Y,groups1)
colnames(tsne_res) <- c("tSNE1", "tSNE2",'SampleID', 'group',"tissue",'time')
...
p <- ggarrange(qc,mc,hc,mix,pre,common.legend = F,legend = 'none',
...
#fig. S1C,D alpha diversity----
library(picante)
source('~/Desktop/molt github/molt plot/alpha.R')
source('~/Desktop/molt github/molt plot/theme_linweichuan.R')
...
#write.csv(data,'~/Desktop//molt github/Fig. S1/alpha result.csv',row.names = F)
####calculate significance
library(reshape2);library(agricolae);library(ggh4x)
data <- read.csv('~/Desktop/molt github/Fig. S1/alpha result.csv')
...
df1 <- subset(df,name=='Shannon'|name=='Richness')
#write.csv(df1,'~/Desktop/molt github/Fig. S1/palpha.csv',row.names = F)

#fig. S1 calculate community stability-----
...
data$name <- 'Stability'
#write.csv(data,'~/Desktop/molt github/Fig. S1/pstability.csv',row.names = F)

#fig. S1C-E plot alpha and stability----
#first rbind combine shannon, richness, and stability into palpha.csv  in excel or R
library(ggh4x)
df1 <- read.csv('~/Desktop/molt github/Fig. S1/palpha.csv')
